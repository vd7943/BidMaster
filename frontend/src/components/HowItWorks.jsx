...
const HowItWorks = () => {
  const steps = [
    ...
    },
    {
      ...
      description:
        "After winning an item, the highest bidder will receive an email with the Auctioneer's payment method information, including bank transfer, Upi Id, and PayPal.",
    },
    {
      icon: <FaDollarSign />,
    ...
    <>
      <section className="w-full ml-0 m-0 h-fit px-5 pt-10 lg:pl-[320px] flex flex-col min-h-screen py-4 justify-center">
        <div className="mx-auto w-full h-auto px-2 flex flex-col gap-4 items-center sm:pt-0 pt-10 justify-center rounded-md">
          <H1 color={"#d6482b"} content={"How BidMaster Works"}></H1>
        </div>
        <div className="flex flex-col gap-4 py-5">
          {steps.map((element, index) => {
            return (
              <div
                key={index}
                className="rounded-md p-2 lg:p-5 flex flex-col gap-2 group hover:bg-black transition-all duration-300"
              >
                <div className="bg-black text-white p-3 text-xl rounded-full w-fit group-hover:bg-[#d64a2b] transition-all duration-300">
                  {element.icon}
                </div>
